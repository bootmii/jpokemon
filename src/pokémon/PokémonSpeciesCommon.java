...
public class PokémonSpeciesCommon {
	...
	/*
	 * The arrays of shorts stat* are ordered as follows: HP, Attack, Defense,
	...
	public void setRandomIVs() {
		if (IVsBuilt)
			throw new IllegalArgumentException("IVs already set");
		// set 6 random ivs from 0 to 32
		for (int i : statIVs)
			statIVs[i] = (short) ThreadLocalRandom.current().nextInt(0, 32);
		IVsBuilt = true;
